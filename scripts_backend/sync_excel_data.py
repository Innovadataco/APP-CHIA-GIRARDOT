...
def sync():
    ...
    for _, row in df_listas.iterrows():
        ...
        if clean_val(row.iloc[13]): 
            se_list.append({"cod": clean_val(row.iloc[13]), "name": clean_val(row.iloc[14]), "cat": clean_val(row.iloc[16])})
        # SUB (index 18, 19, 20, 21)
        if clean_val(row.iloc[18]): 
            sub_list.append({"cod": clean_val(row.iloc[18]), "sigla": clean_val(row.iloc[19]), "name": clean_val(row.iloc[20]), "desc": clean_val(row.iloc[21])})
        # FN (index 23, 24, 25) -> uses 'id' instead of 'cod'
        if clean_val(row.iloc[23]): 
            fn_list.append({"id": clean_val(row.iloc[23]), "name": clean_val(row.iloc[24]), "desc": clean_val(row.iloc[25])})
        # CC (index 27, 28, 29, 30) -> uses 'id' instead of 'cod'
        if clean_val(row.iloc[27]): 
            cc_list.append({"id": clean_val(row.iloc[27]), "sigla": clean_val(row.iloc[28]), "name": clean_val(row.iloc[29]), "cap": clean_val(row.iloc[30])})
    ...
    for s_name in xl.sheet_names:
        if s_name.startswith('EOV-'):
            ...
            e = {"id": s_name, "ico": "rocket_launch", "type": "p"}
            ...
            for _, row in df_e.iterrows():
                ...
                if current_section in ["alertas", "contingencia", "criterios", "evolucion", "kpis"]:
                    ...
                    if v: v = re.sub(r':?\s*VALOR POR DEFINIR;\s*PROPUESTA\s*\((.*?)\)', r': \1', v)
