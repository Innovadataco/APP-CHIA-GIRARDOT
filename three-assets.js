...
const ThreeAssets = {
    ...
    createPMVSystem: (scene, zPos = 20, color = 0x00ff88) => {
        ...
        const screen = new THREE.Mesh(
            ...
            new THREE.MeshStandardMaterial({ map: tex, emissive: 0x3b82f6, emissiveIntensity: 0.2 })
        );
    ...
    createCCTV: (scene, pos) => {
        const group = new THREE.Group();
        const poleMat = new THREE.MeshStandardMaterial({ color: 0x64748b });
        const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.4, 15, 12), poleMat);
        pole.position.y = 7.5;
        const camBody = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.8, 1.2), new THREE.MeshStandardMaterial({ color: 0x1e293b }));
        camBody.position.y = 15;
        group.add(pole, camBody);
        group.position.set(pos.x, 0, pos.z);
        scene.add(group);
        return camBody;
    },

    createCameraFrustum: (scene, pos, color = 0x3b82f6) => {
        const geom = new THREE.CylinderGeometry(0, 8, 12, 4);
        geom.rotateX(Math.PI / 2); // ROTACIÓN NATIVA PARA LOOKAT PRECISO
        const mat = new THREE.MeshPhongMaterial({ color, transparent: true, opacity: 0.15, side: THREE.DoubleSide });
        const frustum = new THREE.Mesh(geom, mat);
        frustum.position.set(pos.x, pos.y, pos.z);
        scene.add(frustum);
        return frustum;
    },

    createAmbulance: (scene) => {
        const group = new THREE.Group();
        const whiteMat = new THREE.MeshStandardMaterial({ color: 0xffffff });
        const redMat = new THREE.MeshStandardMaterial({ color: 0xef4444 });
        
        const body = new THREE.Mesh(new THREE.BoxGeometry(3.5, 3, 7.5), whiteMat);
        body.position.y = 1.5;
        
        const cabinAt = new THREE.Mesh(new THREE.BoxGeometry(3.2, 2.2, 3), whiteMat);
        cabinAt.position.set(0, 1.1, 2.5);
        
        const sRL = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.3, 0.5), new THREE.MeshStandardMaterial({ color: 0xef4444, emissive: 0xef4444, emissiveIntensity: 0 }));
        sRL.position.set(-0.8, 3.1, 1.5);
        const sWL = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.3, 0.5), new THREE.MeshStandardMaterial({ color: 0xfff1f2, emissive: 0xfff1f2, emissiveIntensity: 0 }));
        sWL.position.set(0.8, 3.1, 1.5);
        
        group.add(body, cabinAt, sRL, sWL);
        group.position.set(-8, 0, 40); // Posicionamiento inicial cerca de la víctima
        group.visible = false;
        scene.add(group);
        return { group, sRL, sWL };
    },
