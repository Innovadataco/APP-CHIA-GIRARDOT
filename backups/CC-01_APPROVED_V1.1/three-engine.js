...
class ThreeEngine {
    ...
    getStepMapping(idx) {
        ...
        const maps = {
            ...
            'CC-02': [
                ...
                { target: -150, cam: { x: 280, y: 80, z: -50 }, look: { x: 0, y: 10, z: -50 } }
            ],
            'CC-05': [
        ...
        switch(this.componentId) {
            ...
            case 'CC-02':
                if (idx === 1) {
                    ...
                    // PASO 4: ACTIVACION CAM 2 (PREVENTIVA CCO)
                    if (this.activeFrustum2) {
                        this.activeFrustum2.visible = true;
                        this.activeFrustum2.material.color.set(0xff0000);
                        this.activeFrustum2.material.opacity = 0.4;
            ...
            case 'CC-04':
                if (idx === 1) {
                    ...
                    this.sos.pulse.material.opacity = 0.6;
                    this.sos.pulse.scale.set(1, 1, 1);
                    gsap.to(this.sos.pulse.scale, { x: 30, y: 30, z: 30, duration: 1.5, repeat: -1 });
                    gsap.to(this.sos.pulse.material, { opacity: 0, duration: 1.5, repeat: -1 });
                }
                if (idx === this.steps.length - 1) { 
            ...
            case 'CC-05':
                if (idx === 1) {
                    if(this.vdsV.cone) { 
                        ...
                        gsap.to(this.vdsV.cone.material, { opacity: 0.3, duration: 0.5, repeat: 3, yoyo: true }); 
                    }
                    setTimeout(() => { 
                    ...
                    this.vds.loops.forEach((l, i) => {
                        gsap.to(l.material, { emissiveIntensity: 10, opacity: 1, duration: 0.2, repeat: 3, yoyo: true, delay: i*0.1 });
                    });
                    this.updateCCOStats('VDS_OK: +1', this.stats.total);
                }
                if (idx === this.steps.length - 1) { 
            ...
            case 'CC-11':
                if (idx === 2 && this.pillarMat) {
                    gsap.to(this.pillarMat, { emissiveIntensity: 8, duration: 0.2, repeat: 5, yoyo: true });
                }
                if (idx === this.steps.length - 1) { 
    ...
    drawPMV(pmv, messageType) {
        if (!pmv) return;
        const ctx = pmv.ctx; ctx.fillStyle = '#020617'; ctx.fillRect(0, 0, 512, 256);
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
        ctx.fillStyle = '#3b82f6';
        if (messageType === 'LLUVIA' || messageType === 'ACCIDENTE_VIA') ctx.fillStyle = '#ff4444';
        ctx.font = 'Bold 60px monospace'; ctx.fillText(messageType, 256, 128);
